...
case class PreAllocPingPongBuffer(dataWidth: Int, depth: Int, timeout: Int) extends Component {
  ...
  val buffers = Seq.fill(2)(PreAllocFlushQueue(dataWidth, depth, timeout))

  debug.buffHead0 := buffers(0).debug.head
  debug.buffHead1 := buffers(1).debug.head
