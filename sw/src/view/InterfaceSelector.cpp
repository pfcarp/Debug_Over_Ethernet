...
InterfaceSelector::InterfaceSelector(Sniffer* sniffer): sniffer(sniffer) {
  ...
  // Action
  g_signal_connect(parent, "changed", G_CALLBACK(InterfaceSelector::c_on_combo_changed), nullptr);
}
...
void InterfaceSelector::c_on_combo_changed(GObject* object, GParamSpec* pspec, gpointer user_data) {
